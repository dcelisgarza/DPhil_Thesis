...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		...
		\State{\Comment{Nodal force.}}
		\State{\Comment{Set output index to correspond to whatever \kwd{se} we're on. By parallelising over dislocation lines, we must loop through surface elements in the main code, this is the value of $ k $ we provide. Multiply by 3 and $ N $ because each \kwd{se} has 3 coordinates and $ N $ nodes.}}
...
\begin{subequations}\label{eq:fcne}
	...
	\end{align}
\end{subequations}
where $ e $ is the \kwd{se} label, $ n $ the node label, $ E $ the number of surface elements in the scope. $ N $ the number of nodes in a \kwd{se}.

Data-mapping according to \cref{a:fcne} and relabelling the nodes so they go from $ 0 \to N-1 $, the data from \cref{f:lrse_map} would be arranged in \kwd{devmem} like \cref{eq:fcne_eg},
...
%
The node-coordinate-element (NCE) data mapping in \cref{eq:fnce} is carried out by \cref{a:fnce}, each thread looks after a given \kwd{se}.
\begin{algorithm}
...
\begin{subequations}\label{eq:fnce}
	...
	\end{align}
\end{subequations}
where $ e $ is the surface \textbf{e}lement, $ n $ the \textbf{n}ode, $ E $ the total number of surface elements in scope, $ N $ the total number of nodes in each \kwd{se}.

Data-mapping according to \cref{a:fnce} and relabelling the nodes so they go from $ 0 \to N-1 $, the data from \cref{f:lrse_map} would be arranged in \kwd{devmem} like \cref{eq:fnce_eg},
...
The node mapping would be the same as \cref{a:fcne}. The parallelisation would be on individual packets of a \kwd{se} with a slightly rotated \kwd{dls}, as in a single iteration of the serial code where the \kwd{dls} is rotated. The forces would be averaged with atomic operations and \mintinline{c}{__syncthreads()} before they are added to the total nodal forces.
