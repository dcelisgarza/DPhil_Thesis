...
\begin{enumerate}
    \item The $\mathcal{O}(N^2)$ segment-segment (remote force) calculation is 10\% faster than its equivalent \mintinline{c}{C} function. It is also CPU-parallelised in DDD.jl, which was almost trivial to do, which is definitely not the case for \mintinline{c}{C}. For large numbers of dislocations (1000+ segments) it offers linear scaling with number of threads.
