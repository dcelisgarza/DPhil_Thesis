...
\subsection{Dislocation dynamics in multiscale modelling of materials}

The modelling of materials can be split into different time and length scales \cite{ghoniem2003multiscale,makov2009ab}. In general, smaller scale models inform larger scale ones. Each scale presents its challenges and advantages. Furthermore, the larger the scale of a model, the more approximations and assumptions are made regarding the behaviour system in question.

\begin{figure}
  \centering
  \includegraphics[width=\linewidth]{heirarchy.pdf}
  \caption{Spatial and temporal heirarchy of materials models. Based on \cite{ghoniem2003multiscale}. Scales can overlap with one another and models can be used in conjunction. For example, in this work we used FEM and dislocation dynamics in a multiscale framework.}
  \label{f:multiscale}
\end{figure}

\Cref{f:multiscale} shows the typical spatial and temporal heirarchy of materials models. It must be said that these are broad categories of models and the scale boundaries between each type are by no means clearly defined. All models have a spectrum of scales at which they are useful, their fuzzy spatiotemporal scale boundaries can overlap with other model types. These overlaps can sometimes go beyond what one might expect, such as in this work, where dislocation dynamics is coupled to FEM (continuum mechanics). In fact, previous works by the author during his time as an undergraduate, coupled quantum mechanics to Monte Carlo \cite{celis_garza_2015}, and Monte Carlo to statistical mechanics \cite{celis2015correlations}. Often, the most novel problems and solutions arise from within these scale overlaps, as neither model is the perfect tool to study them, so new methods must be devised.

Dislocation dynamics operates at the mesoscopic level, i.e. about the lengthscale of a crystal grain, on the order of microns to millimitres and mircoseconds to milliseconds. As such, it can provide mechanistic information about what is happening inside a crystal while it is subjected to stress. Dislocation dynamics models are most often directly informed by molecular dynamics. Most commonly, mobility laws, dislocation reactions, and the elasticity model (whether linear, nonlinear, singular or nonsingular) are derived to account for the results of molecular dynamics simulations \cite{fitzgerald2016kink,ddlab,a_non-singular_continuum_theory_of_dislocations,parametricDDD2}. In turn, said molecular dynamics simulations are informed by electronic structure models such as Hartree-Fock and density functional theory. Similarly, dislocation dynamics models go on to inform crystal plasticity finite element models (CPFEM), and traditional finite element methods (FEM).

\subsection{Discrete dislocation dynamics}
\label{ss:ddd}

As previously stated, discrete dislocation dynamics is one of the most common methods for modelling dislocations. In 2D, the dislocations are points with associated Burgers vectors and slip planes, their line directions point toward the observer. 2D dislocation dynamics is akin to taking a slice of the third dimension, so we lose the information of the geometry and topology of the dislocations. In 3D however, this information must be retained. However, we can discretise the dislocations in different ways.

\subsubsection{Parametric DDD}
\label{sss:parametricDDD}

Some of the more exotic models include the \nameref{sss:lvl_set} \cite{ddd_inclusion_as_force} which uses intersecting level-set hyperbolic partial differential equations (PDEs), which is also highly computationally expensive as every step of the simulation requires iteratively solving such PDEs, which often have non-unique solutions.

Another type of parametrisation that yields smooth, continuous dislocation lines are parametric methods \cite{parametricDDD,parametricDDD2}. These use splines \footnote{Cubic splines are commonly used, as the first derivative is smooth and continuous, meaning the velocities will also be smooth and continuous. The second derivative is not smooth, but it is continuous, so kinetic effects will at least be continuous. However, because kinetic effects are often ignored, the lack of smoothness is not a problem.} to describe the smooth curvature of dislocations. These approaches can confer an advantage in reducing the degrees of freedom required to describe a dislocation, as well as being mathematically interesting, and aesthetically pleasing. Unfortunately, they also pose serious challenges in obtaining analytic expressions, therefore their use may require computationally intensive iterative solution methods, quadrature methods, or straight line discretisations of the segments, which defeats the purpose of using such methods in the first place.

\subsubsection{Segment-based/lattice DDD}
\label{sss:segmentDDD}

On the other end of the spectrum lay segment-based\footnote{Also known as lattice-based approach. Where a grid of computational points is defined on a lattice cell which is repeated throughout the domain. The allowed segment orientations and movements are defined using this lattice as anchor points. As the dislocation evolves, the segments are allowed to freely move, but computations are carried out by projecting them onto the nearest set of appropriate lattice points. This makes these codes extremely efficient, but their accuracy depends on how large lattice cells are, how many directions are allowed and the minimum allowable segment lengths.} codes such as microMegas \cite{microMegas,edgeScrewDev} and TRIDIS \cite{fivelTridis}. These only consider specific types of segments, typically pure edge and pure screw. However, in order to account for topological reactions, they also require special junction segments with their associated movement functions and possible interactions. These limit the type and variety of segments they can model, with heuristics for switching between different types. Without mixed segments, one can eliminate the vector projections required to frame mixed dislocations as segments with linear combinations of edge-like and screw-like character. Since most dislocations are of mixed character, this elimiates a lot of computational expense in favour of fast case-switch or if-else statements. Given that junction segments must be specifically defined, general topological functions, such as the ones described in \cref{c:topology} cannot be used. Instead, any reaction that the developers want to make possible, has to be specifically coded in.

These features make segment-based codes very computationally efficient. They also make it easier to create and adopt nonlinear mobilities \cite{edgeScrewDev}; and allow for other simplifications, such as using special cases of the dislocation-induced displacements \cite{fivelDisplacement}, over the full solution for arbitrary loops \cite{bromage2018calculating}.

With extensions to the allowed segment types, they have also been used to the model stochastic behaviour of cross slip \cite{novokshanov20093d}, brittle to ductile transitions \cite{tarleton2007dislocation}, and the relationship between critical stresses required for a loop to overcome an obstacle or cross slip as a function of its size \cite{novokshanov20073d}, which informed \cite{YU2018,yu2020simulating,yu2019influence,fengxian,fengxian2} and directly led to the work in \cref{ss:integrator,ss:matrix,c:topology,c:simulations} as well as help Bruce Bromage develop his improved BCC mobility law.

\subsubsection{Nodal-based DDD}
\label{sss:nodalDDD}

Nodal methods are the third class of DDD discretisations. These are not restricted to specific segment types, and are not based on individual segments. Rather than calculating forces and velocities on individual segments, they are calculated directly on the nodes. They use connectivity information to keep track of how the nodes relate to each other, enabling the calculation of forces and velocities by using shape functions, and summing the contributions from whatever segments are connected to the node.

This approach is used by most modern dislocation dynamics codes such as ParaDiS \cite{paradis}, DDLab \cite{ddlab} and \href{https://github.com/TarletonGroup/EasyDD}{EasyDD}. This method offers more flexibility and is more general than segment-based methods. Topology functions can be generic; they naturally accommodate for any type of mixed segment; mobility functions can be restricted to certain types of segments or movements, gaining back some computational efficiency of segment-based codes, as well as allowing for the easy adoption of nonlinear mobility functions, such as \cite{fitzgerald2016kink}, as a result of the restrictions. They can also rival the smoothness of parametric methods by using higher order shape functions, dynamically remeshing the structure to account for changes in curvature, all the while allowing for analytic expressions to be used. They can also use parametric splines of arbitrary order, in much the same way as parametric methods \cite{po2014recent}.

Due to their flexibility and relative computational efficiency, nodal codes have been used to model a wide range of phenomena including: nanoindentation, cantilever bending, micropillar compression, thin films by coupling dislocation dynamics to finite element methods by the \nameref{sss:superposition} \cite{ferroni2014dislocation,gpu_ddd,bromage2018calculating,po2014recent,sills2016fundamentals}; and the \nameref{sss:discrete_continuum} \cite{dcm0,dcm,dcm_incl}; anisotropic materials---such as the $\alpha$-$\gamma$ phase transition of Fe at 912 $^\circ$C, which is of relevance to fusion and next-generation fission reactors---explicitly \cite{yin2010efficient,fitzgerald2012dislocation,aubry2011equilibrium}, using spherical harmonics \cite{aubry2013use}, and using parametric solutions \cite{han2003parametric}; multijunctions and strain hardening \cite{bulatov2006dislocation}; fundamental junction behaviours \cite{bulatov1998connecting,zbib1998plastic}; the effects of hydrogen \cite{YU2018,yu2020simulating,yu2019influence} on dislocation dynamics; dislocation climb and kinetics \cite{bako2011dislocation,fengxian2}; inclusions, such as ODS steels \cite{fengxian,aubry2019dislocation}; massively parallel simulations \cite{cai2004massively}; size and boundary effects \cite{yasin2001size}; and nonlinear mobility \cite{fitzgerald2016kink,thompson2018interstitial}.

As a result of their flexibility and relative intuitiveness in comparison to other approaches, nodal based codes dominate 3D discrete dislocation dynamics in modern times, particularly as computational capabilities improve.
...
\subsubsection{Continuun dislocation dynamics}

While not strictly discrete dislocation dynamics, continuum dislocation dynamics (CDD) averages the explicit dislocations into ensemble densities and their velocities into ensemble dislocation currents. Each slip system has its own density, current and evolution. The details are outside the scope of this work but are discussed at length in \cite{hochrainer2007three,sandfeld2010numerical,sandfeld2011continuum,hochrainer2014continuum}.

This approach folds explicit dislocation motion and interaction into slip-system averaged densities and velocities via constitutive equations, which are analogous to mobility functions of discrete dislocation dynamics. This approach is entirely compatible with the FEM framework, as the constitutive equations allow the dislocation microstructure to evolve, and from this, the plastic distortion can be directly computed. As \cref{f:cddd_ddd} shows, it can produce some very reasonable results when compared to DDD, provided the callibration is appropriate. However, it also illustrates the fact that we must be careful when making analogies between models, as the systems modelled are very different between simulations.

\begin{figure}
  \centering
  \includegraphics[width=\linewidth]{cddd_ddd.pdf}
  \caption{Comparison of DDD strain profiles and continuum dislocation dynamics for a $\left[1\,0\,0\right]$-oriented FCC bending microbeam. Even though the CDD simulation only contains one slip system, in contrast to their DDD simulation which contained 9, their parametrisation is such that the axial plastic strain of both methods is in good agreement. Image modified from \cite{hochrainer2014continuum}.}
  \label{f:cddd_ddd}
\end{figure}

Despite CDD requiring constitutive equations and careful callibration, the necessary machinery can be easily appended to regular FEM codes. As such, one can use this approach to add functionality to comercial software such as Abaqus, in the form of external subroutines. It is also less computationally intensive than discrete dislocation dynamics, as it does not deal with any of poorly scaling dislocation interaction functions discussed in \cref{c:topology}, which are substantial hurdles that get in the way of large simulations.
...
The internal stress field of a dislocation loop, $L$, in a homogenous, infinite, linear-elastic medium is given by the contour integral around the loop \cite{eigenstrain},
...
\Cref{alg:EasyDD} describes the broad processes by which EasyDD goes through in a simulation. If this project modified or is responsible for a given step, we include a hyperlinked reference in \emph{italics} to the relevant part of the thesis.
\begin{center}
  ...
  \begin{enumerate}
    ...
    \item Couple FEM to DDD via the \nameref{sss:superposition}. \emph{New features: \cref{c:tractions}}.
...
Future improvements that have been identified are described in \cref{c:future}, and a completely new alternative is showcased in \cref{c:DDDjl}.
...
Much of the work presented herein has already proven useful in the simulation of complex simulations such as nano-indentation \cite{YU2018,yu2020simulating,yu2019influence} and several
other research projects within the Tarleton group. \Cref{f:nanoind} is a figure prepared with nanoindentation data from a simulation ran by Haiyang Yu over the course of a week. Without the improvements and additions presented in this work (particularly \cref{c:easydd,c:topology,c:tractions}), this would have been impossible even with months or years of runtime. In fact, Haiyang originally misinterpreted my request, and had been trying to get his simulation to stall during topological operations (collision-separation), as they used to stall so often, but he failed to do so because he did not take advantage of git's version control, and he did not have access to old versions. The figure was created with a loading rate that was one to two orders of magnitude greater than recommended and is not representative of any particular experiment, but even with the increased loading rate, the implications are clear: EasyDD is now much more robust, reliable, and capable than it ever was.

\begin{figure}
  \begin{subfigure}[b]{0.45\linewidth}
    \centering
    \includegraphics[width=\linewidth]{nanoindent.pdf}
    \caption{Reaction force vs indentation depth.}
  \end{subfigure}
  ~
  \begin{subfigure}[b]{0.45\linewidth}
    \centering
    \includegraphics[width=\linewidth]{nanoindentDln.pdf}
    \caption{Prismatic dislocation structures pushed into the domain.}
  \end{subfigure}
  \caption{Simulated loading and unloading of spherical-tipped nanoindentation of a sample. Data and simulations provided by Haiyang Yu.}
  \label{f:nanoind}
\end{figure}

Here we showcase the new capabilities, findings and methods; improvements, fixes and redesign of the codebase; as well as a showcase of relatively simple, yet formerly inaccessible simulations resulting from this work. We then present future plans for continuously improving the codebase. And finally we make a radical and ambitious proposal for future work resulting from the insights gained during the project.
\savearabiccounter
